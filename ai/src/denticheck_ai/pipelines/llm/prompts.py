...
3. 한국어 답변 시 한자(漢字) 및 영어(English) 사용을 절대 금지합니다. 모든 답변은 한글로만 작성해야 합니다.
"""
...
"""
        ...
        return """
[답변 핵심 규칙 - 반드시 엄수할 것]
1. 모든 답변은 100% 한국어(한글)로만 작성하세요. 한자(漢字)와 영어(English) 단어를 절대로 섞어 쓰지 마세요. 전문 용어도 가급적 쉬운 한글로 풀어서 쓰세요.
2. 가독성을 위해 리스트나 목록을 만들 때는 반드시 특수기호 '•' (Bullet point)를 사용하세요. 별표('*')나 대시('-')는 가급적 피하세요.
3. **와 같은 Markdown 강조 기호(Bold)는 절대 사용하지 마세요. 텍스트로만 내용을 전달해야 합니다. (중요: 강조를 위해 별표를 두 개 쓰는 행위는 시스템 파싱 오류를 유발합니다.)
4. 답변 끝에는 전문적인 진료를 위해 치과 방문이 필요함을 부드럽게 강조하세요.
"""
...
주의: 답변에 한자(漢字)와 영어(English)를 절대 사용하지 말고, ** 기호도 사용하지 마세요. '진료'나 '소견'이라는 표현을 사용하고 확정적 '진단'은 피하세요. 모든 답변은 텍스트로만 전달하며 기호 강조는 금지합니다.
"""
