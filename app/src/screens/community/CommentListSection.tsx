...
export function CommentListSection({ comments, onLike, onReply, onEdit, onDelete }: CommentListSectionProps) {
  console.log('[CommentListSection] Rendering - comments count:', comments.length);
  comments.forEach(item => {
    console.log('[CommentListSection] Comment ID:', item.id, 'tags:', item.tags, 'tags count:', item.tags?.length ?? 0);
  });
  ...
  return (
    <View>
      ...
      {comments.length === 0 ? (
        ...
        comments.map((item) => {
          console.log('[CommentListSection] During rendering - Comment ID:', item.id, 'tags:', item.tags);
          return (
            <View key={item.id} className="flex-row gap-3 mb-6">
              ...
              <View className="flex-1 pr-2">
                ...
                {(() => {
                  console.log('[CommentListSection] Tag rendering check - Comment ID:', item.id, 'tags exist:', !!item.tags, 'tags length:', item.tags?.length ?? 0);
                  return null;
                })()}
                {item.tags && item.tags.length > 0 && (
                  <View className="flex-row flex-wrap gap-2 mb-2">
                    {item.tags.map((tag, idx) => {
                      console.log('[CommentListSection] Tag rendering - Comment ID:', item.id, 'tag:', tag);
                      return (
                        <View
                          key={idx}
                ...
                <View className="flex-row items-center gap-3">
                  <TouchableOpacity onPress={() => onReply?.(item.id)}>
                    <Text className="text-xs font-bold text-slate-400">Reply</Text>
                  </TouchableOpacity>
                  {item.isMine && (
                    <>
